...
def plot_raw_data_and_label(raw_data, label, base_filename, save_dir, train_index):
    ...
    try:
        ...
        y_min_dim0, y_max_dim0 = np.min(data_to_plot[:, 0]), np.max(data_to_plot[:, 0])
        ...
        for d in range(n_dims):
            ...
            for start, end in anomaly_segments:
                ...
                if d == 0 and len(valid_seg_indices) > 0:
                    x_pos = (
                        time_index[valid_seg_indices[0]]
                        + time_index[valid_seg_indices[-1]]
                    ) / 2.0
                    y_peak_in_segment = np.max(data_to_plot[valid_seg_indices, 0])
        ...
        if unique_handles:
            ...
            fig.legend(
                ...
                unique_labels,
                loc="lower center",
                ncol=len(unique_handles),  # Arrange horizontally
                # Adjust y value (e.g., 0.99 or 1.0)
                fontsize="small",
                bbox_to_anchor=(0.5, 0.99),
                borderaxespad=0.1,
            )  # Padding between legend and axes

        axes[-1].set_xlabel("Time Index", fontsize=8)
        # Adjust title position slightly if legend is above
        # Lowered y slightly
        fig.suptitle(
            f"Raw Data (Anomalies Highlighted) & Train Split - {base_filename}",
            fontsize=10,
            y=0.97,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="TSB-AD-U-Raw_Data_Plots_AllDims_AnomAnnotated",
    ...
    try:
        ...
        if file_list_df.empty or file_list_df.shape[1] == 0:
            ...
            filenames = file_list_df.iloc[:, 0].astype(str).str.strip().dropna()
    ...
    for i, filename in enumerate(filenames):
        ...
        try:
            ...
            if "Label" not in df.columns:
                print(f"  Warning: Skipping {filename} ('Label' column missing).")
                failed_plots_count += 1
                continue
            if df.shape[1] < 2:
                print(
            ...
            plot_status = plot_raw_data_and_label(
                data, label, base_fname_for_plot, RAW_PLOT_SAVE_DIR, train_index
            )
            if not plot_status:
                failed_plots_count += 1

        except FileNotFoundError:
            print(f"  Error: File not found (outer loop): {file_path}. Skipping.")
            failed_plots_count += 1
        ...
        if temp_label is not None:
            ...
            if temp_anomaly_segments:
                ...
                if temp_segment_strs:
                    temp_anomaly_info_str = "_Anom" + "".join(temp_segment_strs)
        final_expected_path = os.path.join(
            RAW_PLOT_SAVE_DIR,
            f"{base_fname}_Raw_RedAnomaly_AllDims{temp_anomaly_info_str}.png",
        )
    ...
    print(f"Failed during data loading or plotting for {failed_plots_count} files.")
