...
from . import gemini_api_utils_0526, prompt_utils_0526
...
def fill_snippets_by_token_budget(
    ...
    try:
        ...
        count_response_empty = gemini_api_utils_0526.execute_gemini_api_call(
    ...
    while low <= high:
        ...
        try:
            tokens_for_current_selection = gemini_api_utils_0526.execute_gemini_api_call(
                client.models.count_tokens,
                logger,
                model=model_name_for_counting,
    ...
    if selected_snippets:
        try:
            ...
            final_tokens_for_selected = gemini_api_utils_0526.execute_gemini_api_call(
                client.models.count_tokens,
                logger,
                model=model_name_for_counting,
...
def calculate_training_metadata(train_feature_data, train_labels, logger):
    """Calculates metadata for training data (min, max, anomaly indices/values)."""
    if train_feature_data is None or train_labels is None:
        return None
    if len(train_feature_data) == 0:
        return {"min_val": None, "max_val": None, "anomalies": []}

    min_val = float(f"{np.min(train_feature_data):.3g}")
    max_val = float(f"{np.max(train_feature_data):.3g}")
    anomaly_indices = np.where(train_labels == 1)[0]
    anomalies_metadata = []
    for idx in anomaly_indices:
        anomalies_metadata.append({
            "index": int(idx),
            "value": float(f"{train_feature_data[idx]:.3g}")
        })
    
    metadata = {
        "min_val": min_val,
        "max_val": max_val,
        "anomalies": anomalies_metadata # List of dicts {index, value}
    }
    if logger:
        logger.debug(f"Calculated training metadata: Min={min_val}, Max={max_val}, Anomalies Found={len(anomalies_metadata)}")
    return metadata
...
def prepare_training_snippets_for_main_step(
    ...
    # The logic now operates on all available training snippets (temp_full_training_snippets_unprioritized)
    focused_training_snippets_by_range = temp_full_training_snippets_unprioritized
    ...
    if logger:
        logger.debug(
            f"Feature {i_feat}: Prepared {len(source_for_training_snippets_list)} prioritized training snippets (from all available training data) as source for main step batches."
        )
