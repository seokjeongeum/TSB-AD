...
#                     thought_log_paths = []
#                     for part_idx, part in enumerate(response_id_step.candidates[0].content.parts):
...
#                             thought_log_path = os.path.join(temp_artifact_base_dir_for_step, thought_filename)
#                             try:
#                                 with open(thought_log_path, "w") as f_thought:
#                                     f_thought.write(f"Thought Source: Interest ID Step, Feature {{i_feat}}, Key Index {{attempt_key_cycle_idx}}, Same-Key Retry {{same_key_retry_num + 1}}, Part {{part_idx}}\\n---BEGIN THOUGHT---\\n")
...
#                     )
# 
#                     parsed_json = response_id_step.parsed
#                     s_idx, e_idx = parsed_json.get("start_index"), parsed_json.get(
#                         "end_index"
#                     )
# 
#                     if (
...
#                         step_artifacts["parsed_interest_range"] = parsed_range_final
#                         llm_call_succeeded_for_interest_id = True
#                         logger.info(
#                             f"Ft {{i_feat}} Interest ID: Successfully parsed range: {{parsed_range_final}}"
#                         )
#                         break
#                     else:
#                         logger.error(
#                             f"Ft {{i_feat}} Interest ID: Invalid range from LLM: start={{s_idx}}, end={{e_idx}}, n_samples={{n_train_samples}}. Raw: {{response_id_step.parsed}}"
#                         )
#                         logger.debug(
#                             f"LLM response object type: {{type(response_id_step)}}, text: {{response_id_step.text if hasattr(response_id_step, 'text') else 'N/A'}}"
#                         )
#                         raise ValueError(
#                             f"Invalid range from LLM: start={{s_idx}}, end={{e_idx}}"
#                         )
#                 else:
...
#                     if quota_info_id and isinstance(quota_info_id.get("quotaValue"), (int, float)) and \\
#                        quota_info_id.get("quotaMetric") and "input_token" in quota_info_id.get("quotaMetric").lower() and \\
#                        parsed_retry_delay_seconds_id is not None: # Ensure it's retriable
#                         
#                         learned_input_token_quota_for_step = int(float(quota_info_id["quotaValue"]))
#                         logger.info(
...
#                 else:
#                     fatal_keywords_id = [
#                         "api key not valid", "api_key_invalid", "permissiondenied",
#                         # "resource_exhausted", # Not fatal if retryDelay is present
#                         "billing", "developer_inactive",
#                         "consumer_invalid", "api key required"
#                     ]
#                     if any(p in error_lower_str_id for p in fatal_keywords_id):
#                         key_fatal_error_occurred = True
#                     
#                     if not key_fatal_error_occurred and hasattr(e_other_id_call, 'code'):
#                         if e_other_id_call.code in [401, 403]: # Only 401 and 403 are definitively fatal here
#                             key_fatal_error_occurred = True
#                             if not any(p in error_lower_str_id for p in fatal_keywords_id):
#                                  logger.info(f"Ft {{i_feat}} Interest ID: Marking key as fatal due to API status code: {{e_other_id_call.code}}")
#                         elif e_other_id_call.code == 429 and parsed_retry_delay_seconds_id is None: # Check for 429 specifically
...
#                             key_fatal_error_occurred = True
#                             logger.info(f"Ft {{i_feat}} Interest ID: Marking key as fatal due to 429 error WITHOUT a retryDelay.")
#                 
...
#                     if parsed_retry_delay_seconds_id is not None and parsed_retry_delay_seconds_id > 0:
...
#             # The function will return, and MAD_May_26 will handle skipping to the next key from its main list.
#             # No explicit break here needed from `for attempt_key_cycle_idx` as this loop is only one iteration
#             # in the context of how perform_interest_identification_step is called by MAD_May_26.py
