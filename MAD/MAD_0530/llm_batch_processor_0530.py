...
def process_single_batch_with_llm(
    ...
    training_data_indices_info_str,  # Added
    ...
    unpredictability_plot_input_uri: (
        str | None
    ) = None,  # Renamed from reconstructibility_plot_input_uri
    k: int | None = None,  # Changed from characteristic_length_k
):
    """
    ...
    plotting_utils_0530.generate_batch_llm_input_plot(
    ...
    for upload_attempt in range(max_retries_with_delay_per_key):
        try:
            ...
            uploaded_file_response = gemini_api_utils_0530.upload_file_to_gemini(
            ...
            logger.warning(
                f"Batch {batch_num}, Initial Plot Upload Attempt {upload_attempt + 1} failed: {mad_utils_0530._format_exception_for_logging(e_upload)}"
            )
    ...
    batch_model_info = gemini_api_utils_0530.get_gemini_model_info(
        current_attempt_client,
    ...
    batch_input_token_limit = batch_model_info.get(
        "input_token_limit", DEFAULT_FALLBACK_TOKEN_LIMIT
    )

    # --- Prepare training data column for unpredictability baseline guidance ---
    training_feature_col_for_baseline_guidance = None
    if X_train_fit_data is not None and i_feat < X_train_fit_data.shape[1]:
        training_feature_col_for_baseline_guidance = X_train_fit_data[:, i_feat]
    # --- End preparation ---
    ...
    while True:  # Retry indefinitely until code generation and execution succeed
        ...
        for same_key_retry_num in range(
            ...
            while True:
                prompt_for_current_api_call = prompt_utils_0530.construct_llm_batch_prompt(
                    i_feat=i_feat,
                    n_samples=n_samples,
                    ...
                    training_data_indices_info_str=training_data_indices_info_str,
                    ...
                    unpredictability_plot_uri=unpredictability_plot_input_uri,  # Renamed
                    k=k,  # Pass the k parameter
                )
                current_api_call_contents = [prompt_for_current_api_call]
                if image_uri_for_prompt:  # This is the main feature plot URI
                ...
                # NEW: Add unpredictability plot URI to LLM contents if available and distinct from main plot
                if (
                    unpredictability_plot_input_uri
                    and unpredictability_plot_input_uri != image_uri_for_prompt
                ):
                    # Insert it after the main plot (if main plot exists and is first), or at the beginning
                    insert_idx = 0
                    if (
                        image_uri_for_prompt
                        and current_api_call_contents
                        and hasattr(current_api_call_contents[0], "file_data")
                        and current_api_call_contents[0].file_data.file_uri
                        == image_uri_for_prompt
                    ):
                        insert_idx = 1  # Insert after main plot
                    current_api_call_contents.insert(
                        insert_idx,
                        types.Part.from_uri(
                            file_uri=unpredictability_plot_input_uri,
                            mime_type="image/png",
                        ),
                    )
                    logger.info(
                        f"Batch {batch_num}: Added unpredictability plot URI {unpredictability_plot_input_uri} to LLM call contents."
                    )
                ...
                temp_prompt_log_path = mad_utils_0530.log_prompt_text_to_file(
                ...
                try:
                    ...
                    response = gemini_api_utils_0530.execute_gemini_api_call(
                        ...
                        for part_idx, part in enumerate(
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"Failed to save Main LLM Batch thought: {mad_utils_0530._format_exception_for_logging(e_save_thought_batch)}"
                                    )
                    ...
                    if (
                        ...
                        # Log the content when MAX_TOKENS is encountered
                        if response.candidates and response.candidates[0].content:
                            logger.info(
                                f"Batch {batch_num}, Code Gen {current_pass_name}: MAX_TOKENS encountered. Logging response.candidates[0].content: {response.candidates[0].content}"
                            )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Batch {batch_num}, MAX_TOKENS Halving Iter {halving_iteration_this_pass} for pass {current_pass_name}: Failed to re-upload plot. Error: {mad_utils_0530._format_exception_for_logging(e_reupload)}. Using previous or no plot URI."
                            )
                    ...
                    logger.info(
                        ...
                        + f"{' with halved snippets (iteration {halving_iteration_this_pass})' if halving_iteration_this_pass > 0 else ''}"
                    )
                    ...
                    logger.warning(
                        f"Batch {batch_num}, Code Gen {current_pass_name}, API Call Attempt {same_key_retry_num + 1} FAILED. Error: {mad_utils_0530._format_exception_for_logging(e_llm_call)}"
                    )
                    # --- Robust Key Fatal Error Check ---
                    key_is_truly_fatal = False
                    error_lower_str = str(e_llm_call).lower() if e_llm_call else ""
                    parsed_retry_delay_seconds = (
                    ...
                    try:
                        ...
                        logger.warning(
                            "mad_utils.extract_quota_info_from_error_details_json not found. Quota info will be unavailable for this error."
                        )
                    ...
                    if quota_info:
                        ...
                        if (
                            quota_info
                            and isinstance(quota_info.get("quotaValue"), (int, float))
                            and quota_info.get("quotaMetric")
                            and "input_token" in quota_info.get("quotaMetric").lower()
                            and parsed_retry_delay_seconds is not None
                        ):  # Ensure it's retriable

                            learned_input_token_quota_for_batch = int(
                                float(quota_info["quotaValue"])
                            )
                            logger.info(
                    ...
                    if parsed_retry_delay_seconds is not None:
                        ...
                        if not key_is_truly_fatal and hasattr(e_llm_call, "code"):
                            ...
                            if e_llm_call.code in [
                                401,
                                403,
                            ]:  # Only 401 and 403 are definitively fatal here
                                key_is_truly_fatal = True
                                if not any(
                                    p in error_lower_str for p in fatal_keywords
                                ):  # Log if code-based detection was primary
                                    logger.info(
                                        f"Batch {batch_num}, Code Gen {current_pass_name}: Marking key as fatal due to API status code: {e_llm_call.code}"
                                    )
                            elif (
                                e_llm_call.code == 429
                                and parsed_retry_delay_seconds is None
                            ):
                                # 429 is only fatal if there's no specific retry instruction from the API.
                                key_is_truly_fatal = True
                                logger.info(
                                    f"Batch {batch_num}, Code Gen {current_pass_name}: Marking key as fatal due to 429 error WITHOUT a retryDelay (and not caught by 'resource_exhausted' keyword)."
                                )
            ...
            if same_key_retry_num < max_retries_with_delay_per_key - 1:
                ...
                if (
                    parsed_retry_delay_seconds is not None
                    and parsed_retry_delay_seconds > 0
                ):
        ...
        generated_code_str = mad_utils_0530.strip_markdown_code_fences(
            llm_response_text_current_attempt
        )
        current_code_log_path = mad_utils_0530.log_code_to_file(
            generated_code_str,
            batch_extras_save_dir,
        ...
        try:
            ...
            if callable(anomaly_func):
                ...
                if batch_output_artifact_data.get(
                    "code_expects_X_data_col_param", False
                ):
                    logger.info(
                        f"Batch {batch_num}, Code Gen {current_pass_name}: Executing generated code WITH X_data_col parameter."
                    )
                    temp_scores_this_attempt = anomaly_func(
                        X_data_col=X_data_col.copy()
                    )  # Pass a copy
                else:
                    temp_scores_this_attempt = anomaly_func()
                if isinstance(
            ...
            formatted_exec_error = mad_utils_0530._format_exception_for_logging(e_exec)
            logger.error(
        ...
        if not execution_succeeded_this_attempt:
            ...
            plot_success_failed_scores, _ = (
                plotting_utils_0530.generate_llm_identified_anomalies_plot(
                    X_data_col=X_data_col,
            ...
            if plot_success_failed_scores:
                for upload_retry in range(max_retries_with_delay_per_key):
                    try:
                        ...
                        logger.warning(
                            f"Batch {batch_num}, Upload of failed scores plot (for refinement) attempt {upload_retry+1} failed: {mad_utils_0530._format_exception_for_logging(e_upload_fail_plot)}"
                        )
