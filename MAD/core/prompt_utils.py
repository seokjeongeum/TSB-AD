...
1.  **Plot (if image URI provided: {image_uri_for_llm is not None}):**
    ...
    - 'Train Focus Range': A specific segment of training data, where the data line appears in `darkblue`.
...
3.  **Training Snippets (JSON: {training_data_indices_info_str}):**
    Structure: {{ ""value"": {{ ""index_str"": float_value }}, ""label"": {{ ""index_str"": int_label }} }}. Label 1=anomaly, 0=normal.
    ```json
    {training_snippets_json_str}
    ```

4.  **Analysis Snippets (JSON: {analysis_data_indices_info_str}):**
...
3.  **CRITICAL SCORING LOGIC (Plot-Focused with Score Influence):**
    ...
    *   **Identifying and Scoring Anomalies:** For each index `idx_int` corresponding to an entry in `analysis_data_snippets` for THIS BATCH:
        ...
        2.  Visually compare the data line pattern around `idx_int` to learned normal and anomalous patterns.
        3.  If the visual pattern at `idx_int` strongly indicates an anomaly (e.g., a clear spike, dip, shift), this `idx_int` becomes a candidate for a core anomaly. Proceed to the "Scoring Surrounding Points" rule below. If it appears clearly normal, assign a low score (e.g., 0.0 to 0.1) to `anomaly_scores[idx_int]`. If ambiguous, assign an intermediate score reflecting your confidence, but typically this would not trigger the broad influence rule unless confidence becomes high.
    *   **Scoring Surrounding Points (Mandatory Broad Influence for Core Anomalies):** If you identify `idx_int` (from `analysis_data_snippets`) as the center of a visually distinct anomalous event (e.g., a clear spike, dip, or shift inconsistent with normal training patterns):
        1.  **Set `anomaly_scores[idx_int] = 1.0`.**
        2.  **Determine an influence half-width `k`:**
            *   This `k` **MUST be an integer of at least 50** (e.g., `k=50` means total influence of `2*50+1 = 101` points).
            *   The choice of `k` (e.g., typical values 50, 60, up to ~120, or more if strongly supported by visual evidence) should be primarily guided by the visual extent of the anomaly's "area of effect" on the plot. For example, if the plot shows a disturbance lasting for roughly 140 points around `idx_int`, then a `k` value like 70 would be appropriate.
            *   If visual cues for the full extent are ambiguous but `idx_int` is clearly anomalous, use a default `k` such as 60 (total influence 121 points).
...
5.  **Justification Comments:** For EACH distinct anomalous event (originating from an `analysis_data_snippets` index) where you apply the "Mandatory Broad Influence" rule (i.e., score set to 1.0 at `idx_int` and decayed broadly):
    ...
    b.  Explain the visual patterns on the plot justifying why `idx_int` is considered the center of an anomaly.
    c.  Describe your reasoning for the scores assigned to surrounding points: **Confirm that `anomaly_scores[idx_int]` is 1.0. State the chosen half-width `k` (must be >= 50). Explicitly state the start (`idx_int-k`) and end (`idx_int+k`) indices of the ideal influenced range. Explain how visual cues on the plot (e.g., "the visual disturbance extends for roughly 140 points, so k=70 was chosen," or "visual extent was ambiguous, so default k=60 was used") justify this choice of `k`. Confirm that scores decay linearly to 0.01 at these `idx_int +/- k` boundaries (or at array edges if `k` extends beyond them).**
...
def calculate_anomaly_scores():
    ...
    # Plot shows a sharp drop at index 500, inconsistent with normal patterns.
    # Visual inspection suggests the anomalous effect (e.g., slow recovery, volatility)
    # extends for about 120 points in total. Thus, a half-width k=60 is chosen.
    # Primary anomaly point: core_anomaly_idx = 500 (from analysis_data_snippets).
    # Justification: Sharp drop at 500. Training data shows no such drops.
    # Scoring:
    #   - anomaly_scores[500] = 1.0.
    #   - k_half_width = 60    (since 2*60+1 = 121 > 100, and visual effect is ~120 points).
    #   - Scores decay linearly from 1.0 at index 500 to 0.01 at indices 440 (500-60) and 560 (500+60).
    # Refinement Note (example): If a previous attempt used k=30, this refinement increases k to 60 due to wider visual impact.
    
    # Example implementation for the above scenario:
    # core_anomaly_idx = 500 # Assume this came from an analysis snippet
    # k_half_width = 60    # Chosen based on rule (>=50) and visual extent.
    #
