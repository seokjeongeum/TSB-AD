...
class TSPulseChannelFeatureMixerBlock(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        out = inputs + residual.to(inputs.device)
        return out
...
class PatchMixerBlock(nn.Module):
    ...
    def forward(self, hidden_state):
        ...
        if self.self_attn:
            hidden_state = self.norm_attn(hidden_state + x_attn.to(hidden_state.device))

        out = hidden_state + residual.to(hidden_state.device)
        return out
...
class FeatureMixerBlock(nn.Module):
    ...
    def forward(self, hidden: torch.Tensor):
        ...
        out = hidden + residual.to(hidden.device)
        return out
    ...
    def forward(
        ...
        if self.time_masker is not None and enable_masking:
            ...
            if past_observed_mask is not None and self.config.mask_type == "user":
                ...
                assert torch.equal(past_observed_mask, ~mask.to(past_observed_mask.device))
...
class DeviceAgnosticTSPulseForReconstruction(TSPulseForReconstruction):
    """
    ...
    def forward(
        ...
        if return_loss is True:
            if self.config.loss_apply_mode == "full" or (
                ...
                if self.config.loss_apply_mode == "mask":
                    ...
                    masked_reconstruction_loss = self.config.masked_reconstruction_loss_weight * self.loss(
                        reconstruction_outputs.to(bool_mask.device)[bool_mask],
                        past_values.to(bool_mask.device)[bool_mask],
                    )
            ...
            if self.config.fuse_fft and self.config.fft_weight > 0:
                ...
                loss_val = loss_val + fft_loss.to(loss_val.device)
            ...
            if self.config.fuse_fft and self.config.fft_original_signal_loss_weight > 0:
                ...
                if self.config.loss_apply_mode == "mask" and bool_mask.any():
                    reconstructed_ts_from_fft_loss = self.config.fft_original_signal_loss_weight * self.loss(
                        reconstructed_ts_from_fft.to(bool_mask.device)[bool_mask],
                        past_values.to(bool_mask.device)[bool_mask],
                    )
                else:
                    reconstructed_ts_from_fft_loss = self.config.fft_original_signal_loss_weight * self.loss(
                ...
                loss_val = loss_val + reconstructed_ts_from_fft_loss.to(loss_val.device)
            ...
            if self.config.fuse_fft and self.config.enable_fft_prob_loss:
                ...
                loss_val = loss_val + fft_prob_loss.to(loss_val.device)
            ...
            if self.config.fft_time_add_forecasting_pt_loss and future_values is not None:
                ...
                loss_val += forecast_loss.to(loss_val.device)
