...
def run_Unsupervise_AD(model_name, data, **kwargs):
    try:
        ...
        error_message = (
            f"An error occurred while running the model '{function_name}': {str(e)}"
        )
        print(error_message)
        return error_message
...
def run_SAND(data_train, data_test, periodicity=1):
    ...
    clf.fit(
        data_test.squeeze(),
        online=True,
        overlaping_rate=int(1.5 * slidingWindow),
        init_length=len(data_train),
        alpha=0.5,
        batch_size=max(5 * (slidingWindow), int(0.1 * len(data_test))),
    )
    score = clf.decision_scores_
    return score.ravel()
...
def run_Series2Graph(data, periodicity=1):
    ...
    score = np.array(
        [score[0]] * math.ceil(query_length // 2)
        + list(score)
        + [score[-1]] * (query_length // 2)
    )
    return score.ravel()
...
def run_NORMA(data, periodicity=1, clustering="hierarchical", n_jobs=1):
    ...
    slidingWindow = find_length_rank(data, rank=periodicity)
    clf = NORMA(
        pattern_length=slidingWindow, nm_size=3 * slidingWindow, clustering=clustering
    )
    clf.fit(data)
    score = clf.decision_scores_
    score = np.array(
...
def run_AutoEncoder(
    data_train, data_test, window_size=100, hidden_neurons=[64, 32], n_jobs=1
):
...
def run_AnomalyTransformer(
    data_train, data_test, win_size=100, lr=1e-4, batch_size=128
):
...
def run_OFA(data_train, data_test, win_size=100, batch_size=64):
    ...
    clf = OFA(
        win_size=win_size, enc_in=data_test.shape[1], epochs=10, batch_size=batch_size
    )
    clf.fit(data_train)
    score = clf.decision_function(data_test)
    return score.ravel()
